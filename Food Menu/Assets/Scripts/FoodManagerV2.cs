...
public class FoodManagerV2 : MonoBehaviour
{
    ...
    //Until they are all defeated! Obv we might only fight 2 out of the 11 diff types. So we need a new list!
    ...
    void Start()
    {
        ...
        {
            //add a random food item from the buffet to our plate, remember they are already different enemy types!
            GameObject spawnedFood = Instantiate(buffet[Random.Range(0, buffet.Count)], transform);
            plate.Add(spawnedFood);
        }
        //Adctually start setting which food to eat
        SetNewFoodToEat();
    }
    ...
    void EatFood()
    {
        ...
        Destroy(foodToEat);
        //Check if we have eaten all the foods! If we haven't, eat something else.
        if(plate.Count == 0)
        {
            //If we have, we win!
